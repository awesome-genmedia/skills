...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -d '{
    "messages": [
      {
        "role": "user",
        "content": "Create a timelapse starting from this image. Show the passage of time — clouds moving rapidly, lighting changing from morning to evening, shadows sweeping across the scene. Tripod-locked, smooth timelapse motion."
      }
    ],
    "image_urls": ["https://example.com/city-morning.jpg"],
    "stream": false
  }'
```
