...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -d '{
    "messages": [
      {
        "role": "user",
        "content": "Generate a fashion model wearing the garment shown in the image. Full body shot, clean white studio background, natural pose, e-commerce photography style"
      }
    ],
    "image_urls": ["https://example.com/garment-flat-lay.jpg"],
    "stream": false
  }'
```
