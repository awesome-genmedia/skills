...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -d '{
    "messages": [
      {
        "role": "user",
        "content": "Create a new concert poster in the same visual style as this reference. Same color palette and layout structure, but change the subject to a guitarist with stage lights, keep the vintage screenprint texture."
      }
    ],
    "image_urls": ["https://example.com/previous-poster.jpg"],
    "stream": false
  }'
```
