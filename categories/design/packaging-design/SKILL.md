...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -d '{
    "messages": [
      {
        "role": "user",
        "content": "Design new packaging art that extends this brand's visual language. Match the illustration style and color palette, but create a new variation featuring different botanical elements for a new product line. Suitable for a rectangular box panel."
      }
    ],
    "image_urls": ["https://example.com/existing-packaging.jpg"],
    "stream": false
  }'
```
