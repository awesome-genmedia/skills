...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -d '{
    "messages": [
      {
        "role": "user",
        "content": "Create a new logo inspired by this style but for a fintech company called PayFlow. Keep the minimalist aesthetic, use blue tones."
      }
    ],
    "image_urls": ["https://example.com/reference-logo.png"],
    "stream": false
  }'
```
