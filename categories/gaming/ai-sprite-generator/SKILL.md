...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -d '{
    "messages": [
      {
        "role": "user",
        "content": "Create a new enemy sprite sheet matching the art style of this reference. Design a skeleton archer: idle pose, drawing bow, releasing arrow, and taking damage. 4 frames horizontal strip, same pixel size and palette as the reference."
      }
    ],
    "image_urls": ["https://example.com/existing-sprites.png"],
    "stream": false
  }'
```
