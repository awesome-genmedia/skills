...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Write song lyrics inspired by the mood and scene in this image. Capture the emotion, colors, and atmosphere as poetic lyrics with a verse-chorus structure."}],
