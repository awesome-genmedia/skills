...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Generate realistic sound effects that match this scene. Create the ambient soundscape a viewer would hear if they were standing in this location."}],
    "image_urls": ["https://example.com/forest-scene.jpg"],
    "stream": false
  }'
```
