...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Animate this photo: the woman slowly turns her head toward the camera and smiles, her hair gently blowing in the breeze, natural movement"}],
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Animate this portrait: subtle breathing motion, a gentle blink, the person then looks slightly to the left and gives a warm smile. Natural and realistic movement."}],
