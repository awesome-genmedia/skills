...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -d '{
    ...
    "image_urls": ["https://example.com/source-face.jpg", "https://example.com/target-photo.jpg"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Use the face from the first image. Place it onto the person in the second image. The third image shows the desired lighting style to match."}],
    "image_urls": ["https://example.com/face-source.jpg", "https://example.com/body-target.jpg", "https://example.com/lighting-ref.jpg"],
    "stream": false
  }'
```

> `image_urls` is a top-level request parameter (not inside messages). Maximum 4 images. For face swap, you typically need at least 2 images: a source face and a target photo.
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Swap the face from the first image onto the person in the second image. Keep the result natural and photorealistic with smooth edge blending."}],
    "image_urls": ["https://example.com/my-face.jpg", "https://example.com/magazine-cover.jpg"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Place the face from the first image onto the medieval knight character in the second image. Adapt skin tone to match the costume lighting. Keep the helmet and armor unchanged. Result should look like a movie still."}],
    "image_urls": ["https://example.com/actor-headshot.jpg", "https://example.com/knight-scene.jpg"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Take the face from my casual photo (first image) and place it onto the professional business portrait (second image). Match the studio lighting, make the skin tones consistent, and ensure the collar and suit remain untouched."}],
    "image_urls": ["https://example.com/casual-selfie.jpg", "https://example.com/business-portrait-template.jpg"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "In the group photo (second image), swap only the face of the person standing in the center wearing the blue shirt. Use the face from the first image. Keep all other people completely unchanged."}],
    "image_urls": ["https://example.com/replacement-face.jpg", "https://example.com/group-photo.jpg"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Take the face from the first image and place it onto the Renaissance painting portrait in the second image. Blend the face to match the painting style — oil paint texture, period-appropriate lighting, and slightly warm old-master color palette. The face should look like it was part of the original painting."}],
    "image_urls": ["https://example.com/modern-face.jpg", "https://example.com/renaissance-portrait.jpg"],
    "stream": false
  }'
```
