...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Apply the color grading and visual style from this reference image to my video clip"}],
    "image_urls": ["https://example.com/my-video.mp4", "https://example.com/style-reference.jpg"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Apply a cinematic color grade inspired by Blade Runner 2049. Push shadows toward deep blue, highlights toward warm amber. High contrast, slightly desaturated midtones, add atmospheric haze effect."}],
    "image_urls": ["https://example.com/city-footage.mp4"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Convert this video to smooth slow motion at 0.25x speed. Use frame interpolation to keep the motion fluid and artifact-free. Maintain the original resolution."}],
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Transform this real-world video into a Studio Ghibli anime style. Keep the motion and composition, but render everything as hand-painted animation with soft colors and visible brushstrokes."}],
    "image_urls": ["https://example.com/nature-walk.mp4"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Reformat this 16:9 landscape video to 9:16 vertical for TikTok. Intelligently crop to keep the main subject centered. Add a quick zoom-in effect at the beginning for engagement."}],
    "image_urls": ["https://example.com/horizontal-clip.mp4"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Make this video look like it was recorded on a VHS camcorder from the 1990s. Add scan lines, tracking artifacts, slight color bleeding, a date stamp in the corner reading 08/15/1994, and lower the resolution slightly."}],
    "image_urls": ["https://example.com/modern-video.mp4"],
    "stream": false
  }'
```
