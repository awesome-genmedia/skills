...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -d '{
    ...
    "image_urls": ["https://example.com/meeting-recording.mp3"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Transcribe the speech in this video clip. Include timestamps for each sentence."}],
    "image_urls": ["https://example.com/presentation-video.mp4"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Transcribe this meeting recording. Label each speaker (Speaker 1, Speaker 2, etc.). Add timestamps at the start of each speaker turn. Include a brief summary of key decisions and action items at the end."}],
    "image_urls": ["https://example.com/team-meeting.mp3"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Transcribe this video into SRT subtitle format. Each subtitle should be 1-2 lines, maximum 42 characters per line. Format:\n\n1\n00:00:01,000 --> 00:00:04,000\nSubtitle text here\n\n2\n00:00:04,500 --> 00:00:08,000\nNext subtitle here"}],
    "image_urls": ["https://example.com/tutorial-video.mp4"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Transcribe this interview recording. Format as a clean Q&A transcript with the interviewer questions in bold and interviewee answers in regular text. Remove filler words and false starts. Add paragraph breaks for readability."}],
    "image_urls": ["https://example.com/interview.mp3"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "This audio is a Spanish-language podcast episode. Transcribe it in the original Spanish, then provide a complete English translation below. Maintain paragraph structure in both versions."}],
    "image_urls": ["https://example.com/spanish-podcast.mp3"],
    "stream": false
  }'
```
...
curl -X POST https://eachsense-agent.core.eachlabs.run/v1/chat/completions \
  ...
  -H "X-API-Key: $EACHLABS_API_KEY" \
  -d '{
    "messages": [{"role": "user", "content": "Transcribe this podcast episode and generate structured show notes. Include: 1) Episode summary (2-3 sentences), 2) Key topics discussed with timestamps, 3) Notable quotes, 4) Any resources or links mentioned, 5) Full transcript with speaker labels and timestamps."}],
    "image_urls": ["https://example.com/podcast-episode.mp3"],
    "stream": false
  }'
```
